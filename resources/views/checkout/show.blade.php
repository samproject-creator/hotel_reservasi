...
<div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="space-y-6">
            <x-card title="Payment Processing">
                <div class="space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1.5">Amount Paid (Customer Pay)</label>
                        <div class="relative">
                            <span class="absolute inset-y-0 left-0 flex items-center pl-4 text-gray-400 font-bold">Rp</span>
                            <input type="number" name="total_bayar" value="{{ $sisaTagihan }}" min="{{ $sisaTagihan }}" required class="w-full pl-12 pr-4 py-3 bg-white border border-gray-200 rounded-xl focus:outline-none focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 transition-all text-xl font-black">
                        </div>
                        <p class="text-[10px] text-gray-400 mt-2">Enter the amount received from the guest. Change will be calculated automatically.</p>
                    </div>

                    <button type="submit" class="w-full py-4 bg-primary-600 hover:bg-primary-700 text-white font-black rounded-2xl shadow-lg shadow-primary-200 transition-all transform active:scale-[0.98] mt-4 flex items-center justify-center gap-3">
    ...
    <x-card title="Payment Receipt" class="max-w-2xl mx-auto">
        <div class="text-center mb-8 border-b border-dashed border-gray-200 pb-8">
            <h2 class="text-3xl font-black text-gray-900 mb-1 uppercase tracking-tighter">LuxeHotel</h2>
        ...
        <div class="space-y-6">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <p class="font-black text-gray-900">#{{ $booking->kode_booking }}</p>
                </div>
                <div class="text-right">
                    <p class="text-[10px] uppercase font-bold text-gray-400">Date</p>
                    <p class="font-bold text-gray-900">{{ $booking->checkout->waktu_checkout->format('d/m/Y H:i') }}</p>
                </div>
            </div>

            <div class="py-4 border-y border-gray-100 space-y-3">
                <div class="flex justify-between text-sm">
                    <span class="text-gray-500">Accommodation Charge</span>
                    <span class="font-bold text-gray-900">Rp {{ number_format($booking->checkout->total_tagihan, 0, ',', '.') }}</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-gray-500">Method</span>
                    <span class="font-bold text-gray-900 uppercase">{{ $booking->checkout->metode_pembayaran }}</span>
                </div>
            </div>
            ...
            <div class="space-y-2">
                <div class="flex justify-between text-lg">
                    ...
                    <span class="font-black text-gray-900">Rp {{ number_format($booking->checkout->total_bayar, 0, ',', '.') }}</span>
                </div>
            ...
            <div class="bg-primary-50 p-4 rounded-xl flex items-center justify-between">
                <div>
                    <p class="text-primary-700 font-bold">Payment Status</p>
                    <p class="text-xs text-primary-600">Paid in full via {{ $booking->checkout->metode_pembayaran }}</p>
                </div>
            ...
            <div class="pt-8 text-center">
                ...
                <div class="mt-6 flex justify-center gap-4 no-print">
                    <button onclick="window.print()" class="px-6 py-2 bg-gray-900 text-white font-bold rounded-xl hover:bg-gray-800 transition-colors">Print Receipt</button>
                    <a href="{{ route('checkout.index') }}" class="px-6 py-2 bg-gray-100 text-gray-600 font-bold rounded-xl hover:bg-gray-200 transition-colors">Back to List</a>
                </div>
            </div>
        </div>
    </x-card>
