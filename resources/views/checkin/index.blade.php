...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Check-In Management</h1>
    </div>
    ...
    <x-card>
        <form action="{{ route('checkin.index') }}" method="GET" class="mb-6">
            <div class="relative max-w-md">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search guest or booking ID..." class="w-full pl-10 pr-4 py-2 bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-700 dark:text-white rounded-xl focus:outline-none focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 transition-all">
            </div>
        </form>
            ...
            <table class="w-full text-left">
                <thead class="bg-gray-50 dark:bg-gray-800/50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Rooms</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                        <td class="px-6 py-4 text-sm font-bold text-gray-900 dark:text-white">{{ $checkin->kode_booking }}</td>
                        <td class="px-6 py-4">
                            <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ $checkin->tamu->nama_lengkap }}</p>
                            <p class="text-xs text-gray-500 dark:text-gray-400">{{ $checkin->tamu->no_hp }}</p>
                        </td>
                        <td class="px-6 py-4">
