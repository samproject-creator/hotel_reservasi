    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <x-card class="flex flex-col group">
            ...
            <div class="mt-8 pt-8 border-t border-gray-50 dark:border-slate-800 flex items-center gap-3">
                <a href="{{ route('tipe-kamar.edit', $tipe) }}" class="flex-1 px-4 py-2 bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-gray-300 font-black rounded-xl text-[10px] uppercase tracking-widest hover:bg-gray-200 dark:hover:bg-slate-700 transition-colors text-center border border-gray-200 dark:border-slate-700">Edit Class</a>
                <form action="{{ route('tipe-kamar.destroy', $tipe) }}" method="POST" onsubmit="return confirm('Decommission this room classification?')">
                    @csrf @method('DELETE')
                    <button class="p-2 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-colors border border-transparent hover:border-red-100 dark:hover:border-red-900/30"><i data-lucide="trash-2" class="w-5 h-5"></i></button>
                </form>
            </div>
            @endif
        </x-card>
