...
<div class="max-w-4xl mx-auto">
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-black text-gray-900 dark:text-white tracking-tight">Refine Suite Metadata</h1>
            <p class="text-gray-500 dark:text-gray-400 font-medium">Updating global standards for the {{ $tipeKamar->nama_tipe }} class.</p>
        </div>
    ...
    <x-card>
        <form action="{{ route('tipe-kamar.update', $tipeKamar) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-2">
                    ...
                    <input type="number" name="harga_per_malam" id="harga_per_malam" value="{{ old('harga_per_malam', round($tipeKamar->harga_per_malam)) }}"
                        class="w-full px-5 py-3 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 dark:text-white transition-all outline-none" required>
                ...
                <div class="md:col-span-2 space-y-2">
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="4"
                        class="w-full px-5 py-3 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 dark:text-white transition-all outline-none">{{ old('deskripsi', $tipeKamar->deskripsi) }}</textarea>
